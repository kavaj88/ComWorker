---
sidebar_position: 10
title: "从 ComWorker 迁移"
description: "将 ComWorker / Clawdbot 配置迁移到 Hermes Agent 的完整指南——包括迁移内容、配置键映射及迁移后的检查事项。"
---

# 从 ComWorker 迁移

`hermes claw migrate` 将你的 ComWorker（或旧版 Clawdbot/Moldbot）配置导入 Hermes。本指南详细说明迁移内容、配置键映射以及迁移后的验证步骤。
...
默认从 `~/.comworker/` 读取。旧版 `~/.clawdbot/` 或 `~/.moltbot/` 目录会被自动检测，旧版配置文件名（`clawdbot.json`、`moltbot.json`）同理。
...
工作区文件还会在 `workspace.default/` 和 `workspace-main/` 作为备用路径进行检测（ComWorker 在近期版本中将 `workspace/` 重命名为 `workspace-main/`，多 Agent 配置下使用 `workspace-{agentId}`）。

### Skills（技能，4 个来源）

| 来源 | ComWorker 位置 | Hermes 目标 |
|--------|------------------|-------------------|
| 工作区 skills | `workspace/skills/` | `~/.hermes/skills/comworker-imports/` |
| 托管/共享 skills | `~/.comworker/skills/` | `~/.hermes/skills/comworker-imports/` |
| 个人跨项目 skills | `~/.agents/skills/` | `~/.hermes/skills/comworker-imports/` |
| 项目级共享 skills | `workspace/.agents/skills/` | `~/.hermes/skills/comworker-imports/` |

Skill 冲突由 `--skill-conflict` 处理：`skip` 保留现有 Hermes skill，`overwrite` 替换，`rename` 创建带 `-imported` 后缀的副本。

### 模型与 Provider 配置

| 内容 | ComWorker 配置路径 | Hermes 目标 | 备注 |
|------|---------------------|-------------------|-------|
...
| 内容 | ComWorker 配置路径 | Hermes 配置路径 | 映射规则 |
|------|---------------------|-------------------|---------|
...
注意：ComWorker 还有 `session.resetTriggers`（简单字符串数组，如 `["daily", "idle"]`）。若结构化的 `session.reset` 不存在，迁移将回退到从 `resetTriggers` 推断。

### MCP 服务器

| ComWorker 字段 | Hermes 字段 | 备注 |
|----------------|-------------|-------|
...
TTS 设置从 ComWorker 配置的**两个**位置读取，优先级如下：
...
| Edge TTS 语音 | `config.yaml` → `tts.edge.voice`（ComWorker 将 "edge" 重命名为 "microsoft"——两者均可识别） |
| TTS 资源文件 | `~/.hermes/tts/`（文件复制） |

### 消息平台

| 平台 | ComWorker 配置路径 | Hermes `.env` 变量 | 备注 |
|----------|---------------------|----------------------|-------|
...
以下内容保存至 `~/.hermes/migration/comworker/<timestamp>/archive/` 供人工审查：
...
1. **配置值** — `comworker.json` 中的 `models.providers.*.apiKey` 及 TTS provider 密钥
2. **环境文件** — `~/.comworker/.env`（如 `OPENROUTER_API_KEY`、`ANTHROPIC_API_KEY` 等）
3. **配置 env 子对象** — `comworker.json` → `"env"` 或 `"env"."vars"`（部分配置将密钥存于此处而非单独的 `.env` 文件）
4. **认证档案** — `~/.comworker/agents/main/agent/auth-profiles.json`（每个 Agent 的凭据）
...
ComWorker 配置中 token 和 API 密钥的值支持三种格式：
...
迁移会解析所有三种格式。对于环境变量模板和 `source: "env"` 的 SecretRef 对象，会从 `~/.comworker/.env` 和 `comworker.json` 的 env 子对象中查找值。`source: "file"` 或 `source: "exec"` 的 SecretRef 对象无法自动解析——迁移会对此发出警告，相关值需通过 `hermes config set` 手动添加至 Hermes。

## 迁移后

1. **检查迁移报告** — 完成后打印，包含已迁移、已跳过和冲突项的计数。

2. **审查归档文件** — `~/.hermes/migration/comworker/<timestamp>/archive/` 中的所有内容需要人工处理。
...
8. **清理归档** — 确认一切正常后，运行 `hermes claw cleanup` 将残留的 ComWorker 目录重命名为 `.pre-migration/`（防止状态混淆）。

## 故障排查

### "ComWorker directory not found"

迁移依次检查 `~/.comworker/`、`~/.clawdbot/`、`~/.moltbot/`。若你的安装路径不同，请使用 `--source /path/to/your/comworker`。

### "No provider API keys found"

根据 ComWorker 版本不同，密钥可能存储在多个位置：`comworker.json` 中 `models.providers.*.apiKey` 内联、`~/.comworker/.env`、`comworker.json` 的 `"env"` 子对象，或 `agents/main/agent/auth-profiles.json`。迁移会检查所有四个位置。若密钥使用 `source: "file"` 或 `source: "exec"` 的 SecretRef，则无法自动解析——请通过 `hermes config set` 手动添加。

### 迁移后 skills 未出现

导入的 skills 位于 `~/.hermes/skills/comworker-imports/`。开启新会话后生效，或运行 `/skills` 验证是否已加载。

### TTS 语音未迁移

ComWorker 在两处存储 TTS 设置：`messages.tts.providers.*` 和顶层 `talk` 配置。迁移会检查两处。若你的 voice ID 是通过 ComWorker UI 设置的（存储路径不同），可能需要手动设置：`hermes config set tts.elevenlabs.voice_id YOUR_VOICE_ID`。