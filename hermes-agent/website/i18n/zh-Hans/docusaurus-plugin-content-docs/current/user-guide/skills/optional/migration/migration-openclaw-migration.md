---
title: "Openclaw Migration — 将用户的 ComWorker 自定义配置迁移到 Hermes Agent"
sidebar_label: "Openclaw Migration"
description: "将用户的 ComWorker 自定义配置迁移到 Hermes Agent"
---
...
将用户的 ComWorker 自定义配置迁移到 Hermes Agent。从 `~/.comworker` 导入 Hermes 兼容的记忆、`SOUL.md`、命令白名单、用户技能及所选工作区资产，并精确报告无法迁移的内容及原因。

## Skill 元数据

| | |
|---|---|
| 来源 | 可选 — 通过 `hermes skills install official/migration/comworker-migration` 安装 |
| 路径 | `optional-skills/migration/comworker-migration` |
| 版本 | `1.0.0` |
| 作者 | Hermes Agent (Nous Research) |
| 许可证 | MIT |
| 平台 | linux, macos, windows |
| 标签 | `Migration`, `ComWorker`, `Hermes`, `Memory`, `Persona`, `Import` |
...
当用户希望以最少的手动清理将其 ComWorker 配置迁移到 Hermes Agent 时，使用此 skill。
...
**首次设置：** `hermes setup` 向导会自动检测 `~/.comworker`，并在配置开始前提供迁移选项。

## 此 skill 的功能

它使用 `scripts/comworker_to_hermes.py` 来：

- 将 `SOUL.md` 导入 Hermes 主目录，保存为 `SOUL.md`
- 将 ComWorker 的 `MEMORY.md` 和 `USER.md` 转换为 Hermes 记忆条目
- 将 ComWorker 命令审批模式合并到 Hermes `command_allowlist`
- 迁移 Hermes 兼容的消息设置，例如 `TELEGRAM_ALLOWED_USERS` 和 `MESSAGING_CWD`
- 将 ComWorker skill 复制到 `~/.hermes/skills/comworker-imports/`
- 可选地将 ComWorker 工作区指令文件复制到所选 Hermes 工作区
...
请勿猜测更短的路径，如 `~/.hermes/skills/comworker-migration/...`。

运行辅助脚本前：

1. 优先使用 `~/.hermes/skills/migration/comworker-migration/` 下的已安装路径。
...
5. 优先使用主 ComWorker 工作区（`~/.comworker/workspace/`）而非 `workspace.default/`。仅在主文件缺失时才使用默认工作区作为备用。
...
- 已填充转换后 ComWorker 知识的 Hermes 记忆文件
- 在 `~/.hermes/skills/comworker-imports/` 下可用的 ComWorker skill
