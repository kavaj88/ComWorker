...
| 技能 | 描述 |
|-------|-------------|
| [**comworker-migration**](/user-guide/skills/optional/migration/migration-comworker-migration) | 将用户的 ComWorker 自定义配置迁移至 Hermes Agent。从 ~/.comworker 导入兼容 Hermes 的记忆、SOUL.md、命令白名单、用户技能及选定的工作区资产，并报告无法迁移的内容。 |
