...
class AIAgent:
    ...
    def _persist_session(self, messages: List[Dict], conversation_history: List[Dict] = None):
        ...
        """
        # API abort: skip persistence so the aborted run's late result doesn't
        # pollute the session (and appear out of order if the user has
        # continued chatting). The turn-start persistence that already ran
        # before the abort stays; subsequent calls (partial assistant response,
        # checkpoints) are dropped.
        if getattr(self, "_interrupt_suppress_persist", False):
            return
    ...
    def interrupt(self, message: str = None, suppress_persist: bool = False) -> None:
        """
        ...
        Args:
            ...
            suppress_persist: When True, skip ``_persist_session`` on the
                     interrupt path so the aborted run's partial result is not
                     written to the session DB. Used by the API stop endpoint
                     to prevent late persistence from appearing out of order
                     if the user has continued chatting. TUI callers leave
                     this False to preserve partial responses as context.
        ...
        self._interrupt_suppress_persist = suppress_persist
    ...
    def clear_interrupt(self) -> None:
        ...
        self._interrupt_thread_signal_pending = False
        self._interrupt_suppress_persist = False
        if self._execution_thread_id is not None:
