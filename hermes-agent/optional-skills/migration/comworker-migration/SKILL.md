---
name: comworker-migration
description: Migrate a user's ComWorker customization footprint into Hermes Agent. Imports Hermes-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.comworker, then reports exactly what could not be migrated and why.
version: 1.0.0
...
metadata:
  hermes:
    tags: [Migration, ComWorker, Hermes, Memory, Persona, Import]
    related_skills: [hermes-agent]
---

# ComWorker -> Hermes Migration

Use this skill when a user wants to move their ComWorker setup into Hermes Agent with minimal manual cleanup.
...
hermes claw migrate --source /custom/path/.comworker  # Custom source
```
...
**First-time setup:** The `hermes setup` wizard automatically detects `~/.comworker` and offers migration before configuration begins.

## What this skill does

It uses `scripts/comworker_to_hermes.py` to:

- import `SOUL.md` into the Hermes home directory as `SOUL.md`
- transform ComWorker `MEMORY.md` and `USER.md` into Hermes memory entries
- merge ComWorker command approval patterns into Hermes `command_allowlist`
- migrate Hermes-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map ComWorker workspace settings to Hermes working-directory configuration
- copy ComWorker skills into `~/.hermes/skills/comworker-imports/`
- optionally copy the ComWorker workspace instructions file into a chosen Hermes workspace
...
- `scripts/comworker_to_hermes.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.hermes/skills/migration/comworker-migration/scripts/comworker_to_hermes.py`

Do not guess a shorter path like `~/.hermes/skills/comworker-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.hermes/skills/migration/comworker-migration/`.
...
- `{"question":"One or more imported ComWorker skills already exist in Hermes. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the ComWorker workspace instructions file into a Hermes workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
```bash
python3 ~/.hermes/skills/migration/comworker-migration/scripts/comworker_to_hermes.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.hermes/skills/migration/comworker-migration/scripts/comworker_to_hermes.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.hermes/skills/migration/comworker-migration/scripts/comworker_to_hermes.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.hermes/skills/migration/comworker-migration/scripts/comworker_to_hermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.hermes/skills/migration/comworker-migration/scripts/comworker_to_hermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.hermes/skills/migration/comworker-migration/scripts/comworker_to_hermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
5. Prefer the primary ComWorker workspace (`~/.comworker/workspace/`) over `workspace.default/`. Only use the default workspace as fallback when the primary files are missing.
...
- Hermes memory files populated with converted ComWorker knowledge
- ComWorker skills available under `~/.hermes/skills/comworker-imports/`
