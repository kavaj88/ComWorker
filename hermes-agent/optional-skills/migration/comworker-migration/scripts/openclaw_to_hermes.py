#!/usr/bin/env python3
"""ComWorker -> Hermes migration helper.

This script migrates the parts of an ComWorker user footprint that map cleanly
...
SKILL_CATEGORY_DIRNAME = "comworker-imports"
SKILL_CATEGORY_DESCRIPTION = (
    "Skills migrated from an ComWorker workspace."
)
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the ComWorker persona file into Hermes.",
    },
    "workspace-agents": {
        "label": "Workspace instructions",
        "description": "Copy the ComWorker workspace instructions file into a chosen workspace.",
    },
    "memory": {
        "label": "MEMORY.md",
    ...
    "command-allowlist": {
        ...
        "description": "Merge ComWorker exec approval patterns into Hermes command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy ComWorker skills into ~/.hermes/skills/comworker-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
    ...
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared ComWorker skills from ~/.comworker/skills/ into Hermes.",
    },
    "daily-memory": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from ComWorker into Hermes config.yaml.",
    },
    "plugins-config": {
        "label": "Plugins configuration",
        "description": "Archive ComWorker plugin configuration and installed extensions for manual review.",
    },
    "cron-jobs": {
    ...
    "hooks-config": {
        ...
        "description": "Archive ComWorker hook configuration (internal hooks, webhooks, Gmail integration).",
    },
    "agent-config": {
    ...
    "memory-backend": {
        ...
        "description": "Archive ComWorker memory backend settings (QMD, vector search, citations) for manual review.",
    },
    "skills-config": {
        "label": "Skills registry configuration",
        "description": "Archive per-skill enabled/config/env settings from ComWorker skills.entries.",
    },
    "ui-identity": {
        "label": "UI and identity settings",
        "description": "Archive ComWorker UI theme, assistant identity, and display preferences.",
    },
    "logging-config": {
        "label": "Logging and diagnostics",
        "description": "Archive ComWorker logging and diagnostics configuration.",
    },
}
...
# Inspired by ComWorker's src/plugin-sdk/migration.ts so both sides speak the
...
def resolve_secret_input(value: Any, env: Optional[Dict[str, str]] = None) -> Optional[str]:
    """Resolve an ComWorker SecretInput value to a plain string.
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace ComWorker brand names with Hermes in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``ComWorker`` → ``Hermes`` (prose), but lowercase matches
# like ``comworker`` → ``hermes`` (so filesystem paths like ``~/.comworker``
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``ComWorker`` → ``Hermes`` but maps ``comworker`` → ``hermes`` so a
    filesystem path like ``~/.comworker/config.yaml`` rewrites to
...
def rebrand_text(text: str) -> str:
    """Replace ComWorker / ClawdBot / MoltBot brand names with Hermes.
...
#
# Modelled on ComWorker's src/plugin-sdk/migration.ts so both migration tools
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# ComWorker -> Hermes Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        self.output_dir = output_dir or (
            target_root / "migration" / "comworker" / self.timestamp if execute else None
        )
        ...
        # ComWorker's extensions/migrate-hermes/apply.ts "blocked by earlier
        # apply conflict" sequencing.
        self._config_apply_blocked: bool = False

        # Resolve the configured workspace directory from comworker.json.
        # Many users (especially those who started before the ComWorker rebrand)
        # have a custom workspace path (e.g. ~/clawd/) that differs from the
        # default ~/.comworker/workspace/.  Reading agents.defaults.workspace
        # lets source_candidate() find files in the actual workspace.
        self._custom_workspace: Optional[Path] = None
        oc_config = self.load_comworker_config()
    ...
    def source_candidate(self, *relative_paths: str) -> Optional[Path]:
        for rel in relative_paths:
            ...
            if candidate.exists():
                return candidate
            # ComWorker renamed workspace/ to workspace-main/ (and workspace-{agentId}
        ...
        # agents.defaults.workspace in comworker.json.  Users who started
        # before the ComWorker rebrand (when the project was named clawd /
        # clawdbot) often have a custom workspace path outside ~/.comworker/.
    ...
    def migrate(self) -> Dict[str, Any]:
        if not self.source_root.exists():
            self.record("source", self.source_root, None, "error", "ComWorker directory does not exist")
            return self.build_report()

        config = self.load_comworker_config()
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on ComWorker's extensions/migrate-hermes/plan.ts warnings[].
    ...
    def migrate_soul(self) -> None:
        ...
        if not source:
            self.record("soul", None, self.target_root / "SOUL.md", "skipped", "No ComWorker SOUL.md found")
            return
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not source.exists():
            self.record("command-allowlist", None, destination, "skipped", "No ComWorker exec approvals file found")
            return
        if yaml is None:
    ...
    def load_comworker_config(self) -> Dict[str, Any]:
        # Check current name and legacy config filenames
        for name in ("comworker.json", "clawdbot.json", "moltbot.json"):
    ...
    def load_comworker_env(self) -> Dict[str, str]:
        """Load the ComWorker .env file for secrets that live there instead of config."""
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        config = config or self.load_comworker_config()
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # Skip if the workspace points inside the ComWorker source directory —
            # that path will be stale after migration and would cause the Hermes
            # gateway to use the old ComWorker workspace as its cwd, picking up
            # ComWorker's AGENTS.md, MEMORY.md, etc.
            try:
        ...
        if additions:
            self.merge_env_values(additions, "messaging-settings", self.source_root / "comworker.json")
        else:
            self.record("messaging-settings", self.source_root / "comworker.json", self.target_root / ".env", "skipped", "No Hermes-compatible messaging settings found")
    ...
    def handle_secret_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        config_path = self.source_root / "comworker.json"
        if config_path.exists():
            self.record(
                "secret-settings",
                config_path,
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "ComWorker config file not found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            self.merge_env_values(secret_additions, "secret-settings", self.source_root / "comworker.json")
        else:
            self.record(
                "secret-settings",
                self.source_root / "comworker.json",
                self.target_root / ".env",
                "skipped",
    ...
    def _resolve_channel_secret(self, value: Any) -> Optional[str]:
        ...
        return resolve_secret_input(value, self.load_comworker_env())
    ...
    def migrate_discord_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if additions:
            self.merge_env_values(additions, "discord-settings", self.source_root / "comworker.json")
        else:
            self.record("discord-settings", self.source_root / "comworker.json", self.target_root / ".env", "skipped", "No Discord settings found")
    ...
    def migrate_slack_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if additions:
            self.merge_env_values(additions, "slack-settings", self.source_root / "comworker.json")
        else:
            self.record("slack-settings", self.source_root / "comworker.json", self.target_root / ".env", "skipped", "No Slack settings found")
    ...
    def migrate_whatsapp_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if additions:
            self.merge_env_values(additions, "whatsapp-settings", self.source_root / "comworker.json")
        else:
            self.record("whatsapp-settings", self.source_root / "comworker.json", self.target_root / ".env", "skipped", "No WhatsApp settings found")
    ...
    def migrate_signal_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if additions:
            self.merge_env_values(additions, "signal-settings", self.source_root / "comworker.json")
        else:
            self.record("signal-settings", self.source_root / "comworker.json", self.target_root / ".env", "skipped", "No Signal settings found")
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
        ...
        comworker_env = self.load_comworker_env()
        ...
        if isinstance(providers, dict):
            for provider_name, provider_cfg in providers.items():
                ...
                api_key = resolve_secret_input(raw_key, comworker_env)
                if not api_key:
        ...
        # Also check the ComWorker .env file — many users store keys there
        # instead of inline in comworker.json
        comworker_env = self.load_comworker_env()
        env_key_mapping = {
        ...
        for oc_key, hermes_key in env_key_mapping.items():
            val = comworker_env.get(oc_key, "").strip()
            if val and hermes_key not in secret_additions:
                secret_additions[hermes_key] = val

        # Check the comworker.json "env" sub-object — some ComWorker setups
        ...
        if secret_additions:
            self.merge_env_values(secret_additions, "provider-keys", self.source_root / "comworker.json")
        else:
            self.record(
                "provider-keys",
                self.source_root / "comworker.json",
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        config = config or self.load_comworker_config()
        destination = self.target_root / "config.yaml"
        source_path = self.source_root / "comworker.json"

        model_value = config.get("agents", {}).get("defaults", {}).get("model")
        if model_value is None:
            self.record("model-config", source_path, destination, "skipped", "No default model found in ComWorker config")
            return
        ...
        # Resolve a model alias against the ComWorker model catalog.
        # ComWorker stores agents.defaults.model as either a bare string or
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if not isinstance(tts, dict) or not tts:
            self.record("tts-config", source_path, destination, "skipped", "No TTS configuration found in ComWorker config")
            return
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # ComWorker renamed "edge" to "microsoft"; Hermes still uses "edge"
        ...
        # in ComWorker (not messages.tts.elevenlabs directly)
        providers = tts.get("providers") or {}

        # Also check the top-level "talk" config which has provider settings too
        talk_cfg = (config or self.load_comworker_config()).get("talk") or {}
    ...
    def migrate_shared_skills(self) -> None:
        # Check all ComWorker skill sources: managed, personal, project-level
        skill_sources = [
        ...
        if not found_any:
            ...
            self.record("shared-skills", None, destination_root, "skipped", "No shared ComWorker skills directories found")

    def _import_skill_directory(self, source_root: Path, kind_label: str, desc: str) -> None:
        """Import skills from a single source directory into comworker-imports."""
        destination_root = self.target_root / "skills" / SKILL_CATEGORY_DIRNAME
    ...
    def migrate_skills(self) -> None:
        ...
        if not source_root or not source_root.exists():
            self.record("skills", None, destination_root, "skipped", "No ComWorker skills directory found")
            return
    ...
    def archive_docs(self) -> None:
        ...
        partially_extracted = [
            ("comworker.json", "Selected Hermes-compatible values were extracted; raw ComWorker config was not copied."),
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if not mcp_raw:
            self.record("mcp-servers", None, None, "skipped", "No MCP servers found in ComWorker config")
            return
    ...
    def migrate_plugins_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.archive_dir and self.execute:
            ...
            self.record("plugins-config", "comworker.json plugins.*", str(dest), "archived",
                        "Plugins config archived for manual review")
        else:
            self.record("plugins-config", "comworker.json plugins.*", "archive/plugins-config.json",
    ...
    def migrate_cron_jobs(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "comworker.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'hermes cron' to recreate jobs manually.")
            else:
                self.record("cron-jobs", "comworker.json cron.*", "archive/cron-config.json",
    ...
    def migrate_hooks_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.archive_dir and self.execute:
            ...
            self.record("hooks-config", "comworker.json hooks.*", str(dest), "archived",
                        "Hooks config archived for manual review")
        else:
            self.record("hooks-config", "comworker.json hooks.*", "archive/hooks-config.json",
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if defaults.get("thinkingDefault"):
            # Map ComWorker thinking -> Hermes reasoning_effort
        ...
        if changes:
            ...
            if self.execute:
                self.maybe_backup(hermes_cfg_path)
                dump_yaml_file(hermes_cfg_path, hermes_cfg)
            self.record("agent-config", "comworker.json agents.defaults", "config.yaml agent/compression/terminal",
        ...
        if agent_list:
            ...
            self.record("agent-config", "comworker.json agents.list", "archive/agents-list.json",
        ...
        if bindings:
            ...
            self.record("agent-config", "comworker.json bindings", "archive/bindings.json",
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "comworker.json gateway.*", "archive/gateway-config.json",
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # ComWorker uses session.reset (structured) and session.resetTriggers (string array)
        ...
        if changes:
            ...
            if self.execute:
                self.maybe_backup(hermes_cfg_path)
                dump_yaml_file(hermes_cfg_path, hermes_cfg)
            self.record("session-config", "comworker.json session.resetTriggers",
        ...
        if complex_session and self.archive_dir:
            ...
            self.record("session-config", "comworker.json session (advanced)",
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if complex_archive and self.archive_dir:
            ...
            self.record("deep-channels", "comworker.json channels (advanced settings)",
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if changed:
            ...
            if self.execute:
                self.maybe_backup(hermes_cfg_path)
                dump_yaml_file(hermes_cfg_path, hermes_cfg)
            self.record("browser-config", "comworker.json browser.*", "config.yaml browser",
                        "migrated")
        ...
        if advanced and self.archive_dir:
            ...
            self.record("browser-config", "comworker.json browser (advanced)",
    ...
    def migrate_tools_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map exec timeout -> terminal timeout (field is timeoutSec in ComWorker)
        ...
        # Map web search API key (path: tools.web.search.brave.apiKey in ComWorker)
        ...
        if changed and self.execute:
            self.maybe_backup(hermes_cfg_path)
            dump_yaml_file(hermes_cfg_path, hermes_cfg)
            self.record("tools-config", "comworker.json tools.*", "config.yaml terminal",
                        "migrated")
        ...
        if self.archive_dir:
            ...
            self.record("tools-config", "comworker.json tools (full)", "archive/tools-config.json",
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map approval mode (nested under approvals.exec.mode in ComWorker)
        ...
        if mode:
            ...
            if self.execute:
                self.maybe_backup(hermes_cfg_path)
                dump_yaml_file(hermes_cfg_path, hermes_cfg)
            self.record("approvals-config", "comworker.json approvals.mode",
        ...
        if len(approvals) > 1 and self.archive_dir:
            ...
            self.record("approvals-config", "comworker.json approvals (rules)",
    ...
    def migrate_memory_backend(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("memory-backend", "comworker.json memory.*", "archive/memory-backend-config.json",
    ...
    def migrate_skills_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("skills-config", "comworker.json skills.*", "archive/skills-registry-config.json",
    ...
    def migrate_ui_identity(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("ui-identity", "comworker.json ui.*", "archive/ui-identity-config.json",
    ...
    def migrate_logging_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("logging-config", "comworker.json logging/diagnostics",
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# ComWorker -> Hermes Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "",
                "These ComWorker configurations were archived because they don't have a",
        ...
        notes.extend([
            "## IMPORTANT: Archive the ComWorker Directory",
            "",
            "After migration, your ComWorker directory still exists on disk with workspace",
            ...
            "",
            "**Strongly recommended:** Run `hermes claw cleanup` to rename the ComWorker",
            "directory to `.comworker.pre-migration`. This prevents the agent from finding it.",
            ...
            "- Run `hermes claw cleanup` to archive the ComWorker directory (prevents state confusion)",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate ComWorker user state into Hermes Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".comworker"), help="ComWorker home directory")
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   ComWorker -> Hermes Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
