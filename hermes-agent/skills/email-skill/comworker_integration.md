# ComWorker Email Integration

This document explains how to use the email skill from within ComWorker sessions.

## Prerequisites

1. Email configuration file created (e.g., `email_config.json` in your workspace)
2. Python installed and accessible
3. Email skill enabled in ComWorker config

## Using Email in ComWorker Sessions

### Method 1: Direct Python Execution

You can call the email sender directly from ComWorker:

```python
# In an ComWorker session
import sys
import os
...
result = sender.send_email(
    ...
    subject='Test from ComWorker',
    body='This email was sent from ComWorker!',
...
You can use ComWorker's `exec` tool to run the email sender:

```bash
# Send a simple email (from the email skill directory)
cd /path/to/comworker-email-skill
python email_sender.py --to "recipient@example.com" --subject "Hello" --body "Message from ComWorker"
...
Here's a complete function you can add to your ComWorker setup:
...
def send_email_from_comworker(to_email, subject, body, attachments=None, config_path=None):
    """
    Send email from within ComWorker
...
# Example usage in ComWorker:
# result = send_email_from_comworker(
#     to_email='recipient@example.com',
...
def safe_send_email(to_email, subject, body, attachments=None, config_path=None):
    ...
    try:
        result = send_email_from_comworker(to_email, subject, body, attachments, config_path)
