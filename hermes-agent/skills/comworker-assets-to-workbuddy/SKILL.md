---
name: comworker-assets-to-workbuddy
description: 将 ComWorker 用户的个人资产迁移到 WorkBuddy 对应位置，重点覆盖 SOUL.md、IDENTITY.md、USER.md、memory、skills、MCP 配置、bot/channel 连接配置，以及 ComWorker 的 cron job 定义。适用于把 ~/.comworker 或 ComWorker workspace 迁入 ~/.workbuddy、WorkBuddy IDE 配置和目标 WorkBuddy 工作区的场景。
version: 1.0.1
---

# ComWorker 资产迁移到 WorkBuddy

当用户希望把个人 ComWorker 资产迁移到 WorkBuddy 时，使用这个 skill。

默认假设：
- ComWorker home：`~/.comworker`
- ComWorker workspace：`~/.comworker/workspace`
...
- 尤其是 `SOUL.md`、`IDENTITY.md`、`USER.md`，必须优先保留 WorkBuddy 现有人设、语气、边界、偏好，再吸收 ComWorker 中缺失但有价值的信息
- 只有在用户明确要求“以 ComWorker 为准”时，才允许接近覆盖式迁移
- ComWorker `credentials/*` 如果要迁移到 WorkBuddy，默认走“结构化转换”，不是整目录复制
...
迁移：
- `~/.comworker/workspace/SOUL.md` -> `~/.workbuddy/SOUL.md`
- `~/.comworker/workspace/IDENTITY.md` -> `~/.workbuddy/IDENTITY.md`
- `~/.comworker/workspace/USER.md` -> `~/.workbuddy/USER.md`

规则：
- 如果目标文件已存在，优先做 merge，不要粗暴覆盖
- 如果 WorkBuddy 侧内容更丰富，保留现有内容，只补充 ComWorker 中缺失的信息
- `SOUL.md` 的融合优先级最高：先保留 WorkBuddy 已建立的人设、语气、价值观、边界，再补充 ComWorker 中不冲突的设定
- 如果 ComWorker 与 WorkBuddy 的人设设定冲突，不要自动覆盖，保留 WorkBuddy 版本，并把冲突点列入迁移报告
- `IDENTITY.md` 和 `USER.md` 也应采用“补充缺失信息、合并稳定事实、避免重复”的方式处理
- 不要复制 `BOOTSTRAP.md`
- 不要复制 ComWorker workspace state 文件

### 2. Memory

迁移：
- `~/.comworker/workspace/MEMORY.md` 或 `memory.md` -> `{workspace}/.workbuddy/memory/MEMORY.md`
- `~/.comworker/workspace/memory/*.md` -> `{workspace}/.workbuddy/memory/`

规则：
- 日期型 memory 文件如果已经是 `YYYY-MM-DD.md`，原样保留文件名
- 如果 ComWorker 的 `MEMORY.md` 很大，合并到 WorkBuddy `MEMORY.md`，不要整文件直接覆盖
- 两边内容重复时要去重

### 3. Skills

迁移：
- `~/.comworker/skills/<skill>/` -> `~/.workbuddy/skills/<skill>/`
- `~/.comworker/workspace/skills/<skill>/` -> `{workspace}/.workbuddy/skills/<skill>/`
...
主要来源：
- `~/.comworker/comworker.json` 中的 `mcp.servers`

可选来源：
- `~/.comworker/extensions/*/.mcp.json`

目标：
- `~/.workbuddy/mcp.json`，顶层字段使用 `mcpServers`

规则：
- 把 ComWorker 的 `mcp.servers` 转成 WorkBuddy 的 `mcpServers`
- 写入时合并到已有 `~/.workbuddy/mcp.json`，不要覆盖其他 server
- 如果从 ComWorker 插件的 `.mcp.json` 中提取，只迁移 MCP server 定义，不迁移整个插件目录
...
- ComWorker `credentials/*` 如果是为了迁移 bot/channel，应优先迁移到 WorkBuddy IDE 的 `settings.json`，也就是 `claw.channels.*`
- 除非用户明确要求迁移 WorkBuddy 产品账号登录态，否则不要把 ComWorker `credentials/*` 强行写进 `SecretStorage`
...
ComWorker `credentials/*` 到 WorkBuddy 的对照原则：

- `~/.comworker/credentials/oauth.json`
  - 这是 ComWorker 的渠道/OAuth 旧状态来源之一
...
- `~/.comworker/credentials/<channel>-pairing.json`
...
- `~/.comworker/credentials/<channel>-allowFrom.json`
  - 语义上优先映射到：
  - `claw.channels.<type>.allowFrom`
  - 或 `groupAllowFrom` / `groups` / `dmPolicy` / `groupPolicy`
  - 这属于“规则迁移”，不是文件迁移

- `~/.comworker/credentials/<channel>-<accountId>-allowFrom.json`
...
- `~/.comworker/credentials/whatsapp/<accountId>/creds.json`
  - 当前已确认的 WorkBuddy channel 类型列表里，没有内建 `whatsapp`
  - 不要自动迁移到 `claw.channels.*`
  - 默认结论：`manual review / unsupported`

- `~/.comworker/credentials/matrix/credentials.json`
- `~/.comworker/credentials/matrix/credentials-<account>.json`
...
当前没有在这份内建列表里确认到的 ComWorker 渠道，例如：
- WhatsApp
- Matrix
- Telegram
- Signal
...
- ComWorker 无直接对应 WorkBuddy channel 的渠道
...
迁移规则：
- 不要把整个 `~/.comworker/credentials/` 目录整包复制到 WorkBuddy
...
来源：
- `~/.comworker/cron/jobs.json`

目标：
- `~/.workbuddy/automations/<automation_id>/automation.toml`

这里只迁移“定义”，不要迁移：
- `~/.comworker/cron/runs/*.jsonl`
- `~/.comworker/tasks/runs.sqlite`

字段映射：

- ComWorker `enabled: true` -> WorkBuddy `status = "ACTIVE"`
- ComWorker `enabled: false` -> WorkBuddy `status = "PAUSED"`
- ComWorker job id -> WorkBuddy automation id
- ComWorker job name -> WorkBuddy automation name
- ComWorker payload 文本 -> WorkBuddy automation prompt
...
- `payload.kind = "systemEvent"`
  ...
  - 在迁移报告里说明：这和 ComWorker main-session cron 的执行语义并不完全等价
...
### ComWorker cron jobs.json 参考骨架

ComWorker 源文件通常是一个 `jobs` 数组，单个 job 大致可按下面的形态理解：
...
ComWorker 可能出现的 `schedule.kind`：
- `at`
- `every`
- `cron`

ComWorker 可能出现的 `payload.kind`：
- `agentTurn`
- `systemEvent`
...
- `~/.comworker/credentials/`
- `~/.comworker/extensions/` 整个插件目录
- `~/.comworker/.env` 整个文件
- `~/.comworker/comworker.json` 整个文件
- `.clawhub/lock.json`
- ComWorker session logs 和运行态状态文件
...
在读取任何 ComWorker 数据之前，必须先调用 `ask_followup_question` 获得用户确认。
...
```
即将从 ~/.comworker 复制以下内容到 WorkBuddy：
...
2. ComWorker 内容里哪些是稳定事实、补充设定、可兼容偏好
3. 哪些内容重复
4. 哪些内容冲突
...
- 哪些 ComWorker channel 被迁移到了 `claw.channels.*`
- 哪些是写入 `settings.json`
