...
#   - On ComWorker/BoxAI/others: This script is the primary reporting mechanism.
#
...
detect_platform() {
    ...
    # CodeBuddy indicators
        ...
        platform="codebuddy"
    elif [ -n "${COMWORKER_SHELL:-}" ] || [ -n "${COMWORKER_ENV:-}" ] || [ -n "${COMWORKER_VERSION:-}" ]; then
        # COMWORKER_SHELL is ComWorker's officially documented marker set by the
        # exec context (e.g. COMWORKER_SHELL=exec) — it's the most reliable
        # signal we can get, so keep it first among ComWorker env checks.
        platform="comworker"
    ...
    if [ "$platform" = "unknown" ]; then
        ...
        if echo "$parent_cmd" | grep -qi "claude"; then
            ...
            platform="codebuddy"
        elif echo "$parent_cmd" | grep -qi "comworker"; then
            platform="comworker"
    ...
    # ComWorker would otherwise be misclassified as codebuddy.
    if [ "$platform" = "unknown" ]; then
        if [ -d ".claude" ]; then
            platform="claude-code"
        fi
