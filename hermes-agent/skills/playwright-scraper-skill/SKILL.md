...
description: Playwright-based web scraping ComWorker Skill with anti-bot protection. Successfully tested on complex sites like Discuss.com.hk.
...
A Playwright-based web scraping ComWorker Skill with anti-bot protection. Choose the best approach based on the target website's anti-bot level.
...
Use ComWorker's built-in `web_fetch` tool:

```bash
# Invoke directly in ComWorker
...
If the site doesn't have dynamic loading, use ComWorker's `web_fetch` tool—it's fastest.
...
- [ ] Integration with ComWorker's `browser` tool
