...
- **ComWorker展示** - 检测 `COMWORKER_SESSION=1` 时输出 `[COMWORKER_SEND_FILE]路径[/COMWORKER_SEND_FILE]`
...
### 非阻塞交互式登录（AI助手/ComWorker场景）

适用于AI助手或ComWorker场景：**发送二维码给用户后立即结束脚本**，不阻塞主会话。用户扫码确认后，通过后续命令继续完成登录。
...
**ComWorker 自动识别：**
当检测到 `COMWORKER_SESSION=1` 环境变量时，脚本会自动输出 `[COMWORKER_SEND_FILE]路径[/COMWORKER_SEND_FILE]`，ComWorker 可直接在会话中展示图片。
