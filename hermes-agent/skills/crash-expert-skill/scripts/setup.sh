...
#   2. 检测并注册 aicrasher MCP server 到 CodeBuddy / Claude Code / ComWorker
...
check_mcp_server_registered() {
    # 检查 MCP server 是否已注册到 CodeBuddy / Claude Code / ComWorker(mcporter)
...
detect_cli_tool() {
    ...
    # 2. 自动检测: codebuddy > claude > mcporter(ComWorker)
...
register_mcp_server() {
    ...
    # 优先使用 codebuddy，其次 claude，最后 mcporter(ComWorker)
    local CLI_CMD=""
...
full_install() {
    ...
    echo ""
    info "使用方法："
    echo "  1. 在 CodeBuddy / Claude Code / ComWorker 中打开内核源码目录"
