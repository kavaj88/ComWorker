...
> - **ComWorker (mcporter)**: `mcporter config add aicrasher --stdio "python3 -m aicrasher.mcp_server" --scope home`

> **⚠️ ComWorker 环境关键注意事项：`lifecycle` 必须设为 `keep-alive`**
>
...
> ```
> 配置修改后执行 `comworker gateway restart` 使配置生效。
...
> - **必须提示用户退出当前会话并重新启动**（退出后重新运行 `codebuddy` / `claude` / `comworker`），新会话启动时会自动发现并连接已注册的 MCP Server
>
> **处理流程：**
...
>    请退出当前会话，重新启动 CodeBuddy / Claude Code / ComWorker，然后重新执行 vmcore 分析命令。
