...
> ⚠️ **Agent 注意**：用户客户端无法渲染 Markdown 图片，**禁止直接输出上方图片语法**。需向用户发送二维码时，执行 `### 3.9 MCP KEY 与配置` 中的 `comworker message send` 命令发图。
...
编辑 `~/.comworker/comworker.json`，添加：
...
```json
{
  "skills": {
    "entries": {
      "didi-ride-skill": {
        "enabled": true,
        "apiKey": "你的MCP_KEY"  // apiKey 是 ComWorker 标准字段名，存储的值就是滴滴平台的 MCP KEY
      }
    }
  }
}
...
首次使用时，ComWorker 会提示安装 mcporter 工具。
...
2. **检查 Key**：执行 `comworker config get skills.entries.didi-ride-skill.apiKey`，若输出为空或非 `__COMWORKER_REDACTED__`，按 `### 3.9 MCP KEY 与配置` 流程引导。Key 缺失时 mcporter 的报错信息具有误导性，不要尝试绕过。
   - ⚠️ **若 Key 已配置（返回 `__COMWORKER_REDACTED__`）但 mcporter 仍报 `Missing KEY parameter`**：**不是 Key 失效**，**禁止向用户索要 Key**。排查步骤见 `references/error_handling.md` 中的「mcporter Missing KEY parameter」章节。
   - ⚠️ **`__COMWORKER_REDACTED__` 是"已配置"哨兵值、不是真实 Key**：禁止从 `comworker config get`（含 `--raw`）提取 Key 字面量；URL 中 `?key=...` 必须固定用 `$DIDI_MCP_KEY`。误把哨兵拼进 URL 会触发 `SSE error: Invalid content type`，详见 error_handling.md 同名章节。
...
发单成功后（实时单或预约单），根据本章节下文 "comworker cron add" 命令，创建 cron 任务，务必按照本章节下文bash命令参数设置。
...
> **术语说明**：滴滴平台称此凭证为「MCP KEY」，ComWorker 配置字段统一叫 `apiKey`，注入后的环境变量为 `DIDI_MCP_KEY`——三者是同一个值。通过 `comworker config set` 持久化后，ComWorker 在每次 agent run 启动时自动注入为环境变量。

#### 3.9.1 检查 Key 状态

```bash
# 仅用于判断 DIDI_MCP_KEY 是否已配置；输出不是 Key 值，不可代入 URL
comworker config get skills.entries.didi-ride-skill.apiKey
```
结果输出为空 = 未配置；输出 `__COMWORKER_REDACTED__` = 已配置，**必须用**环境变量 `$DIDI_MCP_KEY` 拼接 URL。

⚠️ `comworker config get`（含 `--raw`）永远不返回真实 Key——任何把哨兵值拼入 URL/header/参数的请求都会失败，见 error_handling.md「SSE error / apiKey 占位符泄漏」。
...
comworker config set 'skills.entries.didi-ride-skill.apiKey' 'YOUR_KEY'
export DIDI_MCP_KEY='YOUR_KEY'
```

- 持久化后 ComWorker 在所有后续 agent run（含 cron isolated session）中自动注入 `DIDI_MCP_KEY`
...
```bash
comworker message send --channel {CHANNEL_NAME} --target {CHAT_ID} --media "https://s3-yspu-cdn.didistatic.com/mcp-web/qrcode/didi_ride_skill_qrcode.png" --message "滴滴出行APP扫码获取MCP Key，解锁一键打车"
```

2. 输出文字：

> 您还没有配置 DIDI_MCP_KEY 或 Key 已失效，请访问 [滴滴MCP平台](https://mcp.didichuxing.com/claw) 获取 MCP KEY，然后配置环境变量或在 ComWorker 配置文件中设置。
