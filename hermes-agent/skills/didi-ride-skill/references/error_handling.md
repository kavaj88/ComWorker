...
> `<skill-dir>` 代表 didi-ride-skill 技能的安装根目录（即 SKILL.md 所在目录），可通过 `comworker skills info didi-ride-skill` 获取。
...
2. **当前 shell 未注入环境变量**：comworker 在每次 agent run 启动时自动注入 `DIDI_MCP_KEY`，但手动在终端直接运行 mcporter 时该变量不存在。执行 `echo $DIDI_MCP_KEY` 验证——若为空，在终端手动 `export DIDI_MCP_KEY=<key>` 后重试，或改在 comworker agent 环境中调用。
...
4. **Key 本身确实无效**：若以上均排除，执行 `comworker config get skills.entries.didi-ride-skill.apiKey` 确认 Key 已配置，若返回空则按 `### 3.9 MCP KEY 与配置` 流程重新配置。

***

## SSE error / apiKey 占位符泄漏

mcporter 报 `SseError: SSE error: Invalid content type, expected "text/event-stream"` 时，最常见根因是把 ComWorker 的哨兵值 `__COMWORKER_REDACTED__` 当成真实 Key 拼进了 MCP URL。

**症状**：

- 拼出来的 URL 实际是 `https://mcp-dev.didichuxing.com/mcp-servers?key=__COMWORKER_REDACTED__`
- 服务端把它当成无效 Key 拒绝，返回 HTML / 纯文本错误页（不是 SSE 流）
- mcporter 收到非 `text/event-stream` 响应直接抛 `SseError`

**为什么会拼错**：误以为 `comworker config get skills.entries.didi-ride-skill.apiKey` 配上 `--raw` / `awk` / `sed` 之类的手段能拿到真实 Key 字面量。实际上：

> `comworker config get`（含 `--raw`）**永远不会**返回真实 Key 字面量。`__COMWORKER_REDACTED__` 是"已配置"的哨兵值，不是 Key 本身。真实 Key 由 ComWorker 在每次 agent run 启动时注入到环境变量 `DIDI_MCP_KEY`，调用方只能通过 `$DIDI_MCP_KEY` 使用。
...
3. 若 `env_missing`：当前不在 comworker agent run 上下文里（例如手动终端调用），按 SKILL.md §3.9 引导用户重新配置；或临时 `export DIDI_MCP_KEY=<key>` 后重试。

**禁止**：

- 禁止用 `comworker config get ... --raw` / `awk` / `sed` / `jq` 等任何手段尝试从配置提取 Key 字面量
- 禁止把 `__COMWORKER_REDACTED__` 出现在任何 URL / header / 参数中
