...
export function planVersionBump(rootDir, nextVersion) {
  ...
  operations.push(
    ...
    {
      filePath: path.join(root, 'scripts', 'pack-comworker-skill.mjs'),
      type: 'text',
      apply() {
        return updateTextFile(this.filePath, (source) => {
          let output = replaceRegex(source, /version:\s*'[^']+'/, `version: '${nextVersion}'`, 'pack-comworker version');
          output = replaceRegex(output, /packageName:\s*'fbs-bookwriter-v\d+-comworker'/, `packageName: 'fbs-bookwriter-v${nextCompact}-comworker'`, 'pack-comworker packageName');
          return output;
        });
      },
    }
