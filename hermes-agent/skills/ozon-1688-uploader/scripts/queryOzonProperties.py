...
def get_ozon_api_key():
    ...
    if not key:
        print("Error: OZON_API_KEY not set. Configure it in ComWorker:\n"
...
def get_ozon_client_id():
    ...
    if not key:
        print("Error: OZON_CLIENT_ID not set. Configure it in ComWorker:\n"
...
def main(external_category_id: str = None) -> str:
    """
    主入口函数 - ComWorker SKILL 调用此函数

    ComWorker 框架会将大模型提取的作为参数传入。

    Args:
        external_category_id: 类目数据，通过1688的商品类目映射出来的Ozon类目列表，例如："100,329023"

    Returns:
        str: JSON格式的处理结果字符串（ComWorker要求返回字符串）
    """
