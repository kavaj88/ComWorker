...
1. **获取 token（API key）** — 打开 [www.ctrip.com/wendao/comworker](https://www.ctrip.com/wendao/openclaw)，按页面指引申请并复制你的 **API token**（仅保存在本人可信环境，勿截图含完整密钥发到公开渠道）。
2. **提供 token（二选一）**
...
1. **取得 token**：在 [www.ctrip.com/wendao/comworker](https://www.ctrip.com/wendao/openclaw) 按指引申请并复制 API token。
2. **设置环境变量 `WENDAO_API_KEY`**（变量名须完全一致）：
   ...
   - **ComWorker / 托管环境**：在平台为技能配置的环境变量中填写 `WENDAO_API_KEY`，确保运行时 `process.env.WENDAO_API_KEY` 可用。
