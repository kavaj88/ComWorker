...
```bash
python3 ~/.comworker/skills/meituan-huisheng-coupon/scripts/auth.py token-verify
```
...
```bash
python3 ~/.comworker/skills/meituan-huisheng-coupon/scripts/diag_auth_log.py
```
...
```bash
python3 ~/.comworker/skills/meituan-huisheng-coupon/scripts/diag_issue_log.py
```
...
再执行发券：
```bash
python3 ~/.comworker/skills/meituan-huisheng-coupon/scripts/issue.py --token <上一步获取的token>
```
...
import json
from pathlib import Path
config = json.loads((Path.home()/'.comworker/skills/meituan-huisheng-coupon/scripts/config.json').read_text())
