...
def _resolve_legacy_auth_file() -> Path:
    ...
    注意：其他潜在历史位置（如 ~/.comworker/workspace/auth_tokens.json）
...
def _get_legacy_auth_files() -> list:
    ...
    2. ~/.xiaomei-workspace/mt_auth_tokens.json (小美搭档默认)
    3. ~/.comworker/workspace/auth_tokens.json (ComWorker 默认)
    """
    import os
    files = []
    ...
    # 4. ComWorker 默认路径（保留旧文件名）
    comworker_path = Path.home() / ".comworker" / "workspace" / "auth_tokens.json"
    if comworker_path not in files:
        files.append(comworker_path)
...
def _detect_platform() -> str:
    ...
    # 检测 ClawHub
    if os.environ.get("CLAWHUB_ENV") or os.environ.get("COMWORKER_AVAILABLE"):
        return "clawhub"

    # 默认检测 comworker 命令是否存在
    try:
...
def cmd_cron_set(time_str: str):
    ...
    if platform == "claude":
        ...
        result["schedule_action"] = "comworker_cron"
        # 获取当前用户ID（优先从环境变量获取）
        user_id = os.environ.get("CLAWHUB_USER_ID") or os.environ.get("USER_ID") or "${USER_ID}"
        result["schedule_config"] = {
            "command": f"comworker cron add --name '美团每日自动领券' --cron '{cron_expr}' --tz Asia/Shanghai --message '帮我领美团券，领完通知我' --channel daxiang --to user:{user_id} --announce --timeout-seconds 120 --session isolated",
