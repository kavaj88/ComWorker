...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: ComWorker native memory → Hermes + Honcho."""
    from pathlib import Path

    # ── Detect ComWorker native memory files ──────────────────────────────────
    cwd = Path(os.getcwd())
    comworker_home = Path.home() / ".comworker"
    ...
    print("\nHoncho migration: ComWorker native memory → Hermes\n" + "─" * 50)
    print()
    print("  ComWorker's native memory stores context in local markdown files")
    ...
    print()
    print("Step 2  Detected ComWorker memory files")
    print()
    if user_files or agent_files:
        ...
        print("  No ComWorker native memory files found in cwd or ~/.comworker/.")
    ...
    print("  agent's character, capabilities, and behavioral rules. In ComWorker")
    ...
    # ── Step 5: What changes ──────────────────────────────────────────────────
    print()
    print("Step 5  What changes vs. ComWorker native memory")
    print()
    print("  Storage")
    print("    ComWorker: markdown files on disk, searched via QMD at prompt-build time.")
    ...
    print("    ComWorker: file excerpts injected synchronously before each LLM call.")
    ...
    print()
    print("  Memory growth")
    print("    ComWorker: you edit files manually to update memory.")
    ...
    print("    ComWorker: no persistent session concept — files are global.")
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from comworker-honcho to Hermes Honcho",
    )
