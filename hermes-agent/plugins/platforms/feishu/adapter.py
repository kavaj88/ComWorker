...
- Per-chat serial message processing (matches comworker createChatQueue)
- Processing status reactions: Typing while working, removed on success,
  swapped for CrossMark on failure
- Reaction events routed as synthetic text events (matches comworker)
- Interactive card button-click events routed as synthetic COMMAND events
- Webhook anomaly tracking (matches comworker createWebhookAnomalyTracker)
- Verification token validation as second auth layer (matches comworker)
...
_FEISHU_DEDUP_TTL_SECONDS = 24 * 60 * 60          # 24 hours — matches comworker
...
_FEISHU_WEBHOOK_RATE_LIMIT_MAX = 120               # max requests per window per IP — matches comworker
...
_FEISHU_WEBHOOK_ANOMALY_TTL_SECONDS = 6 * 60 * 60  # anomaly tracker TTL (6 hours) — matches comworker
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    async def _handle_message_with_guards(self, event: MessageEvent) -> None:
        ...
        time (matches comworker's createChatQueue serial queue behaviour).
        """
    ...
    def _record_webhook_anomaly(self, remote_ip: str, status: str) -> None:
        ...
        Mirrors comworker's createWebhookAnomalyTracker: TTL 6 hours, log every 25 consecutive
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        # Rate limiting — composite key: app_id:path:remote_ip (matches comworker key structure).
        ...
        # Verification token check — second layer of defence beyond signature (matches comworker).
    ...
    def _check_webhook_rate_limit(self, rate_key: str) -> bool:
        ...
        The rate_key is composed as "{app_id}:{path}:{remote_ip}" — matching comworker's key
