...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def run(self):
        ...
        try:
            ...
            pass
        # First-time ComWorker-residue banner — fires once if ~/.comworker/ exists
        # after an ComWorker→Hermes migration (especially migrations done by
        # ComWorker's own tool, which doesn't archive the source directory).
        try:
            from agent.onboarding import (
            ...
            if not is_seen(self.config, COMWORKER_RESIDUE_FLAG) and detect_comworker_residue():
                try:
                    _resid_color = _welcome_skin.get_color("banner_dim", "#B8860B")
                except Exception:
                    _resid_color = "#B8860B"
                self._console_print(f"[{_resid_color}]{comworker_residue_hint_cli()}[/]")
                try:
                    from hermes_cli.config import get_config_path as _get_cfg_path_resid
                    mark_seen(_get_cfg_path_resid(), COMWORKER_RESIDUE_FLAG)
                except Exception:
