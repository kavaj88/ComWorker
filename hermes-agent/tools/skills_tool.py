...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    for scan_dir in dirs_to_scan:
        # Per-directory catalog.json: when present it is the authoritative list
        # of skills for that directory and provides their category. This keeps
        # classification config-driven (add a category = edit config, not code)
        # and stops nested SKILL.md files from being treated as standalone skills.
        _cat_map: dict = {}
        _cat_file = scan_dir / "catalog.json"
        if _cat_file.is_file():
            try:
                _cd = json.loads(_cat_file.read_text(encoding="utf-8"))
                if isinstance(_cd, dict) and _cd.get("skills"):
                    _cat_map = _cd["skills"]
        ...
        for skill_md in iter_skill_index_files(scan_dir, "SKILL.md"):
            ...
            try:
                ...
                if name in disabled:
                    continue
                # Catalog present for this dir -> only load listed skills.
                _cat_entry = _cat_map.get(name)
                if _cat_map and _cat_entry is None:
                    continue
                ...
                category = (_cat_entry or {}).get("category") or _get_category_from_path(skill_md)
