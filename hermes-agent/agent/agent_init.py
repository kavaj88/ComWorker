...
def init_agent(
    ...
    # API abort sets this to skip _persist_session so the aborted run's late
    # result doesn't pollute the session (and appear out of order if the user
    # has continued chatting). TUI interrupts leave it False to preserve the
    # partial response as context for the next turn.
    agent._interrupt_suppress_persist = False
