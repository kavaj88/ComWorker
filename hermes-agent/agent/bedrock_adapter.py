...
Reference: ComWorker's ``extensions/amazon-bedrock/`` plugin, which implements
...
# Priority order matches ComWorker's resolveAwsSdkEnvVarName():
...
def resolve_aws_auth_env_var(env: Optional[Dict[str, str]] = None) -> Optional[str]:
    ...
    This mirrors ComWorker's ``resolveAwsSdkEnvVarName()`` — used to detect
...
def has_aws_credentials(env: Optional[Dict[str, str]] = None) -> bool:
    ...
    This two-tier approach mirrors the pattern from ComWorker PR #62673:
...
def discover_bedrock_models(
    ...
    Mirrors ComWorker's ``discoverBedrockModels()`` in
...
# ---------------------------------------------------------------------------
# Mirrors ComWorker's classifyFailoverReason() and matchesContextOverflowError()
