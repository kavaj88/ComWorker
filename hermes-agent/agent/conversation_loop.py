...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        try:
            ...
            # Pull from reasoning_content/reasoning/inline <think> tags via
            # _extract_reasoning. Using assistant_message.content here sent the
            # answer text as reasoning — the streaming ProcessCard then hid it
            # (isSimilarText matched the answer delta) while the final state
            # showed the real reasoning_content, so the two diverged.
