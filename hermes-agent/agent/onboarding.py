...
COMWORKER_RESIDUE_FLAG = "comworker_residue_cleanup"
...
def comworker_residue_hint_cli() -> str:
    """Banner shown the first time Hermes starts and finds ``~/.comworker/``.
    ...
    the old directory — with a warning that archiving breaks ComWorker.
    """
    return (
        "A legacy ComWorker directory was detected at ~/.comworker/.\n"
        "To port your config, memory, and skills over to Hermes, run "
        "`hermes claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `hermes claw cleanup` (renames it to ~/.comworker.pre-migration — "
        "ComWorker will stop working after this).\n"
        "This tip only shows once."
    )


def detect_comworker_residue(home: Optional[Path] = None) -> bool:
    """Return True if an ComWorker workspace directory is present in ``$HOME``.

    Pure filesystem check — no side effects. ``home`` override exists for tests.
    """
    base = home or Path.home()
    try:
        return (base / ".comworker").is_dir()
    except OSError:
        return False
