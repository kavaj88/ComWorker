...
def test_run_conversation_codex_continues_after_ack_stop_message(monkeypatch):
    ...
    responses = [
        _codex_ack_message_response(
            "Absolutely — I can do that. I'll inspect ~/comworker-studio and report back with a walkthrough."
        ),
        _codex_tool_call_response(),
        _codex_message_response("Architecture summary complete."),
    ]
    ...
    result = agent.run_conversation("look into ~/comworker-studio and tell me how it works")
    ...
    assert any(
        msg.get("role") == "assistant"
        and msg.get("finish_reason") == "incomplete"
        and "inspect ~/comworker-studio" in (msg.get("content") or "")
        for msg in result["messages"]
    )
    assert any(
        msg.get("role") == "user"
