...
from agent.onboarding import (
    BUSY_INPUT_FLAG,
    COMWORKER_RESIDUE_FLAG,
    TOOL_PROGRESS_FLAG,
    busy_input_hint_cli,
    busy_input_hint_gateway,
    detect_comworker_residue,
    is_seen,
    mark_seen,
    comworker_residue_hint_cli,
...
# ---------------------------------------------------------------------------
# ComWorker residue banner
# ---------------------------------------------------------------------------


class TestDetectOpenclawResidue:
    def test_returns_true_when_comworker_dir_present(self, tmp_path):
        (tmp_path / ".comworker").mkdir()
        assert detect_comworker_residue(home=tmp_path) is True

    def test_returns_false_when_absent(self, tmp_path):
        assert detect_comworker_residue(home=tmp_path) is False

    def test_returns_false_when_path_is_a_file(self, tmp_path):
        # A stray file named ``.comworker`` is NOT a workspace — skip the banner.
        (tmp_path / ".comworker").write_text("oops")
        assert detect_comworker_residue(home=tmp_path) is False

    def test_default_home_does_not_crash(self):
        # Smoke: real $HOME lookup must not raise regardless of state.
        assert isinstance(detect_comworker_residue(), bool)
...
class TestOpenclawResidueHint:
    def test_hint_mentions_migrate_command(self):
        ...
        msg = comworker_residue_hint_cli()
        assert "hermes claw migrate" in msg
        assert "~/.comworker" in msg

    def test_hint_mentions_cleanup_command(self):
        # `cleanup` is mentioned as the follow-up archive step.
        assert "hermes claw cleanup" in comworker_residue_hint_cli()

    def test_hint_warns_cleanup_breaks_comworker(self):
        # Archiving the directory breaks ComWorker for users still running it —
        # the banner must flag that side effect.
        msg = comworker_residue_hint_cli().lower()
        assert "comworker will stop working" in msg or "stop working" in msg

    def test_hint_not_empty(self):
        assert comworker_residue_hint_cli().strip()
...
class TestOpenclawResidueSeenFlag:
    def test_flag_independent_of_other_flags(self, tmp_path):
        ...
        mark_seen(cfg_path, BUSY_INPUT_FLAG)
        loaded = yaml.safe_load(cfg_path.read_text())
        assert is_seen(loaded, COMWORKER_RESIDUE_FLAG) is False

    def test_flag_round_trips(self, tmp_path):
        cfg_path = tmp_path / "config.yaml"
        assert mark_seen(cfg_path, COMWORKER_RESIDUE_FLAG) is True
        loaded = yaml.safe_load(cfg_path.read_text())
        assert is_seen(loaded, COMWORKER_RESIDUE_FLAG) is True
