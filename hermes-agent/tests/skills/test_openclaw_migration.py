...
def test_migrator_optionally_imports_supported_secrets_and_messaging_settings(tmp_path: Path):
    ...
    (source / "comworker.json").write_text(
        json.dumps(
            {
                "agents": {"defaults": {"workspace": "/tmp/comworker-workspace"}},
    ...
    assert "MESSAGING_CWD=/tmp/comworker-workspace" in env_text
...
def test_messaging_cwd_skipped_when_inside_source(tmp_path: Path):
    """MESSAGING_CWD pointing inside the ComWorker source dir should be skipped."""
    mod = load_module()
    source = tmp_path / ".comworker"
    target = tmp_path / ".hermes"
    target.mkdir()
...
def test_source_candidate_finds_files_in_custom_workspace(tmp_path: Path):
    """When agents.defaults.workspace points outside ~/.comworker, files should
    ...
    # No workspace/ directory inside .comworker — files live in custom workspace
...
def test_source_candidate_prefers_standard_workspace_over_custom(tmp_path: Path):
    """When files exist in both ~/.comworker/workspace/ and the custom workspace,
...
def test_shared_skills_migrated(tmp_path: Path):
    """Shared skills from ~/.comworker/skills/ are migrated."""
    mod = load_module()
    source = tmp_path / ".comworker"
    target = tmp_path / ".hermes"
    target.mkdir()
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    """Bug fix: archive cron store even when comworker.json has no top-level cron config."""
    mod = load_module()
    source = tmp_path / ".comworker"
    target = tmp_path / ".hermes"
    output_dir = target / "migration-report"
    ...
    (source / "comworker.json").write_text(json.dumps({"channels": {}}), encoding="utf-8")
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    result = skills_guard.scan_skill(
        ...
        source="official/migration/comworker-migration",
    )
...
def test_rebrand_text_replaces_comworker_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Hermes``.
    assert mod.rebrand_text("ComWorker prefers Python 3.11") == "Hermes prefers Python 3.11"
    ...
    assert mod.rebrand_text("COMWORKER uses tools well") == "Hermes uses tools well"
    # All-lowercase matches → lowercase ``hermes``; this preserves the
    # real filesystem path ``~/.hermes`` (Hermes home) when rebranding
    # memory entries that reference ``~/.comworker`` or ``comworker`` prose.
    assert mod.rebrand_text("comworker should always respond concisely") == "hermes should always respond concisely"
...
def test_rebrand_text_handles_multiple_replacements():
    mod = load_module()
    text = "ComWorker said to ask ClawdBot about MoltBot settings"
...
def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.comworker`` filesystem paths — must
    rewrite to lowercase ``.hermes`` (the real Hermes home), not the broken
    ``.Hermes``.

    Regression test for @versun's ComWorker-residue feedback: after migration,
    memory entries that referenced ``~/.comworker/config.yaml`` were being
    rewritten to ``~/.Hermes/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.comworker/config.yaml") == \
        "config is at ~/.hermes/config.yaml"
    assert mod.rebrand_text("use .comworker directory") == "use .hermes directory"
    assert mod.rebrand_text("Path.home() / '.comworker'") == "Path.home() / '.hermes'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("comworker config path: ~/.comworker/") == \
        "hermes config path: ~/.hermes/"


def test_migrate_memory_rebrands_entries(tmp_path):
    mod = load_module()
    source_root = tmp_path / "comworker"
    source_root.mkdir()
    workspace = source_root / "workspace"
    workspace.mkdir()
    memory_md = workspace / "MEMORY.md"
    memory_md.write_text(
        "# Memory\n\n- ComWorker should use Python 3.11\n- ClawdBot prefers dark mode\n",
        encoding="utf-8",
    )
    ...
    assert "ComWorker" not in result
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    soul_md.write_text("You are ComWorker, an AI assistant made by SparkLab.", encoding="utf-8")
...
def _run_model_migration(tmp_path: Path, comworker_json: dict) -> dict:
    """Helper: run just migrate_model_config on an comworker.json and return
    ...
    (source / "comworker.json").write_text(json.dumps(comworker_json), encoding="utf-8")
...
def test_migrate_model_config_resolves_alias_against_real_comworker_schema(tmp_path: Path):
    """Regression for #16745 — ComWorker's catalog is keyed by the full
