"""Tests for the ComWorker→Hermes migration hardening features.
...
def _load():
    spec = importlib.util.spec_from_file_location("comworker_to_hermes_hard", SCRIPT_PATH)
...
def test_redact_leaves_env_secretref_alone():
    ...
    # If we later want to exempt SecretRef values the way ComWorker does, update
...
def _make_minimal_migrator(mod, tmp_path, **overrides):
    ...
    source.mkdir()
    # Minimal valid ComWorker layout so the Migrator constructor doesn't choke.
    (source / "comworker.json").write_text("{}", encoding="utf-8")
...
def test_json_mode_redacts_secrets_in_output(tmp_path):
    ...
    source.mkdir()
    (source / "comworker.json").write_text("{}", encoding="utf-8")
    # Plant a fake ComWorker .env with a recognizably-shaped key.
