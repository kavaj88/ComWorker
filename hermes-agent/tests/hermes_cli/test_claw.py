...
class TestFindMigrationScript:
    ...
    def test_finds_project_root_script(self, tmp_path):
        script = tmp_path / "comworker_to_hermes.py"
        script.write_text("# placeholder")
        with patch.object(claw_mod, "_COMWORKER_SCRIPT", script):
    ...
    def test_finds_installed_script(self, tmp_path):
        ...
        with (
            patch.object(claw_mod, "_COMWORKER_SCRIPT", tmp_path / "nonexistent.py"),
            patch.object(claw_mod, "_COMWORKER_SCRIPT_INSTALLED", installed),
        ):
            assert claw_mod._find_migration_script() == installed

    def test_returns_none_when_missing(self, tmp_path):
        with (
            patch.object(claw_mod, "_COMWORKER_SCRIPT", tmp_path / "a.py"),
            patch.object(claw_mod, "_COMWORKER_SCRIPT_INSTALLED", tmp_path / "b.py"),
        ):
            assert claw_mod._find_migration_script() is None


# ---------------------------------------------------------------------------
# _find_comworker_dirs
# ---------------------------------------------------------------------------


class TestFindOpenclawDirs:
    """Test discovery of ComWorker directories."""

    def test_finds_comworker_dir(self, tmp_path):
        comworker = tmp_path / ".comworker"
        comworker.mkdir()
        with patch("pathlib.Path.home", return_value=tmp_path):
            found = claw_mod._find_comworker_dirs()
        assert comworker in found
...
class TestArchiveDirectory:
    ...
    def test_adds_timestamp_when_archive_exists(self, tmp_path):
        ...
        (tmp_path / ".comworker.pre-migration").mkdir()
...
class TestCmdMigrate:
    ...
    def _mock_comworker_running(self):
        with patch.object(claw_mod, "_detect_comworker_processes", return_value=[]):
            yield
    ...
    def test_error_when_script_missing(self, tmp_path, capsys):
        comworker_dir = tmp_path / ".comworker"
        comworker_dir.mkdir()
        args = Namespace(
            source=str(comworker_dir),
            dry_run=True, preset="full", overwrite=False,
            migrate_secrets=False, workspace_target=None,
            skill_conflict="skip", yes=False,
        )
    ...
    def test_dry_run_succeeds(self, tmp_path, capsys):
        ...
        fake_mod = ModuleType("comworker_to_hermes")
    ...
    def test_dry_run_does_not_touch_source(self, tmp_path, capsys):
        ...
        assert comworker_dir.is_dir()  # Source untouched
    ...
    def test_full_preset_does_not_enable_secrets_silently(self, tmp_path, capsys):
        ...
        even under the 'full' preset.  This mirrors ComWorker's migrate-hermes
...
class TestCmdCleanup:
    ...
    def test_no_dirs_found(self, tmp_path, capsys):
        ...
        with patch.object(claw_mod, "_find_comworker_dirs", return_value=[]):
            claw_mod._cmd_cleanup(args)
        captured = capsys.readouterr()
        assert "No ComWorker directories found" in captured.out

    def test_dry_run_lists_dirs(self, tmp_path, capsys):
        comworker = tmp_path / ".comworker"
        comworker.mkdir()
        ws = comworker / "workspace"
        ws.mkdir()
        (ws / "todo.json").write_text("{}")

        args = Namespace(source=None, dry_run=True, yes=False)
        with patch.object(claw_mod, "_find_comworker_dirs", return_value=[comworker]):
            claw_mod._cmd_cleanup(args)

        captured = capsys.readouterr()
        assert "Would archive" in captured.out
        assert comworker.is_dir()  # Not actually archived

    def test_archives_with_yes(self, tmp_path, capsys):
        comworker = tmp_path / ".comworker"
        comworker.mkdir()
        (comworker / "workspace").mkdir()
        (comworker / "workspace" / "todo.json").write_text("{}")
        ...
        assert not comworker.exists()
        assert (tmp_path / ".comworker.pre-migration").is_dir()
    ...
    def test_skips_when_user_declines(self, tmp_path, capsys):
        ...
        args = Namespace(source=None, dry_run=False, yes=False)
        with (
            patch.object(claw_mod, "_find_comworker_dirs", return_value=[comworker]),
            patch.object(claw_mod, "prompt_yes_no", return_value=False),
            patch("sys.stdin", mock_stdin),
        ):
            claw_mod._cmd_cleanup(args)
        ...
        assert comworker.is_dir()

    def test_explicit_source(self, tmp_path, capsys):
        custom_dir = tmp_path / "my-comworker"
        custom_dir.mkdir()
    ...
    def test_handles_multiple_dirs(self, tmp_path, capsys):
        ...
        args = Namespace(source=None, dry_run=False, yes=True)
        with patch.object(claw_mod, "_find_comworker_dirs", return_value=[comworker, clawdbot]):
            claw_mod._cmd_cleanup(args)
...
class TestPrintMigrationReport:
    ...
    def test_execute_report(self, capsys):
        report = {
            ...
            "output_dir": "/home/user/.hermes/migration/comworker/20250312T120000",
        }
...
class TestDetectOpenclawProcesses:
    ...
    def test_returns_match_on_windows_when_comworker_exe_running(self):
        with patch.object(claw_mod, "sys") as mock_sys:
            mock_sys.platform = "win32"
            with patch.object(claw_mod, "subprocess") as mock_subprocess:
                mock_subprocess.run.side_effect = [
                    MagicMock(returncode=0, stdout="comworker.exe                 1234 Console    1     45,056 K\n"),
                ]
                result = claw_mod._detect_comworker_processes()
                assert len(result) >= 1
                assert any("comworker.exe" in r for r in result)

    def test_returns_match_on_windows_when_node_exe_has_comworker_in_cmdline(self):
        with patch.object(claw_mod, "sys") as mock_sys:
            mock_sys.platform = "win32"
            with patch.object(claw_mod, "subprocess") as mock_subprocess:
                mock_subprocess.run.side_effect = [
                    MagicMock(returncode=0, stdout=""),  # tasklist comworker.exe
...
class TestWarnIfOpenclawRunning:
    def test_noop_when_not_running(self, capsys):
        with patch.object(claw_mod, "_detect_comworker_processes", return_value=[]):
            claw_mod._warn_if_comworker_running(auto_yes=False)
        captured = capsys.readouterr()
        assert captured.out == ""

    def test_warns_and_exits_when_running_and_user_declines(self, capsys):
        with patch.object(claw_mod, "_detect_comworker_processes", return_value=["comworker process(es) (PIDs: 1234)"]):
        ...
        captured = capsys.readouterr()
        assert "ComWorker appears to be running" in captured.out
    ...
    def test_warns_and_continues_in_auto_yes_mode(self, capsys):
        with patch.object(claw_mod, "_detect_comworker_processes", return_value=["comworker process(es) (PIDs: 1234)"]):
            claw_mod._warn_if_comworker_running(auto_yes=True)
        captured = capsys.readouterr()
        assert "ComWorker appears to be running" in captured.out
