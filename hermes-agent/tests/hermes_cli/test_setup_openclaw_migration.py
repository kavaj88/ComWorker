"""Tests for ComWorker migration integration in the setup wizard."""
...
# ---------------------------------------------------------------------------
# _offer_comworker_migration — unit tests
# ---------------------------------------------------------------------------


class TestOfferOpenclawMigration:
    """Test the _offer_comworker_migration helper in isolation."""

    def test_skips_when_no_comworker_dir(self, tmp_path):
        """Should return False immediately when ~/.comworker does not exist."""
        with patch("hermes_cli.setup.Path.home", return_value=tmp_path):
            assert setup_mod._offer_comworker_migration(tmp_path / ".hermes") is False
    ...
    def test_skips_when_migration_script_missing(self, tmp_path):
        ...
        with (
            ...
            patch.object(setup_mod, "_COMWORKER_SCRIPT", tmp_path / "nonexistent.py"),
        ):
            assert setup_mod._offer_comworker_migration(tmp_path / ".hermes") is False
    ...
    def test_skips_when_user_declines(self, tmp_path):
        ...
        with (
            patch("hermes_cli.setup.Path.home", return_value=tmp_path),
            patch.object(setup_mod, "_COMWORKER_SCRIPT", script),
    ...
    def test_runs_migration_when_user_accepts(self, tmp_path):
        ...
        with (
            ...
            result = setup_mod._offer_comworker_migration(hermes_home)
    ...
    def test_creates_config_if_missing(self, tmp_path):
        ...
        with (
            ...
            setup_mod._offer_comworker_migration(hermes_home)
...
class TestSetupWizardOpenclawIntegration:
    """Verify _offer_comworker_migration is called during first-time setup."""

    def test_migration_offered_during_first_time_setup(self, tmp_path):
        """On first-time setup, _offer_comworker_migration should be called."""
        ...
        with (
            ...
            patch.object(
                setup_mod, "_offer_comworker_migration", return_value=False
            ) as mock_migration,
    ...
    def test_migration_reloads_config_on_success(self, tmp_path):
        ...
        with (
            ...
            patch("builtins.input", return_value=""),
            patch.object(setup_mod, "prompt_choice", return_value=1),
            patch.object(setup_mod, "_offer_comworker_migration", return_value=True),
            patch.object(setup_mod, "setup_model_provider"),
            patch.object(setup_mod, "setup_terminal_backend"),
            patch.object(setup_mod, "setup_agent_settings"),
            patch.object(setup_mod, "setup_gateway"),
...
class TestGetSectionConfigSummary:
    ...
    # force ComWorker migration to re-run setup for an already-configured
    # section.
...
class TestSetupWizardSkipsConfiguredSections:
    ...
    def test_sections_skipped_when_migration_imported_settings(self, tmp_path):
        ...
        with (
            ...
            patch.object(
                setup_mod, "_offer_comworker_migration",
