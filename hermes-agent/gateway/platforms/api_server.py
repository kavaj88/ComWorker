...
# ── MCP OAuth/CLI relay adapter ─────────────────────────────────────────────
# The relay's core logic lives in hermes_cli.mcp_oauth_relay as FastAPI
# handlers. We adapt those handlers to aiohttp here because the platform
# gateway proxies /api/comworker/mcp/* to THIS api_server (port 18080), not to
# the (non-running in `gateway run` mode) web_server FastAPI app. The handlers
# raise fastapi.HTTPException on error, which we catch and translate to aiohttp
# responses; the OAuth callback returns a fastapi HTMLResponse, which we also
# translate. Imported lazily/guarded so a missing relay module never breaks
# the API server.
try:
    from hermes_cli.mcp_oauth_relay import (  # type: ignore
        oauth_start as _relay_oauth_start,
        oauth_callback as _relay_oauth_callback,
        oauth_poll as _relay_oauth_poll,
        cli_install as _relay_cli_install,
        cli_auth as _relay_cli_auth,
        cli_status as _relay_cli_status,
    )
    from fastapi import HTTPException as _FastAPIHTTPException
    from fastapi.responses import HTMLResponse as _FastAPIHTMLResponse
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _create_agent(
        ...
        # Per-request model override (e.g. the model the client selected in
        # the chat UI). Falls back to the gateway default model when absent.
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        # Stream reasoning content as message.delta events so the client can
        # render the thinking process token-by-token (instead of only a single
        # end-of-turn `reasoning.available`). The client's Run SSE handler
        # consumes `payload.reasoning_content` on `message.delta` events, and
        # the platform sanitizer passes `message.delta` blocks without a
        # `delta` field through unchanged. This mirrors the TUI gateway's
        # `reasoning.delta` wiring, but uses the message.delta shape the
        # web client actually expects.
    ...
    async def _handle_run_events(self, request: "web.Request") -> "web.StreamResponse":
        ...
        try:
            while True:
                ...
                # Batching multiple events into a single response.write()
                # avoids a transport-level race where back-to-back writes
                # within ~1ms can silently drop the first event's bytes.
                events = [event]
                while len(events) < 20:
                    try:
                        ev = q.get_nowait()
                ...
                if not parts:
                    if stream_closed:
                        break
                    continue
                await response.write(b"".join(parts))
                if stream_closed:
                    break
    ...
    async def _handle_stop_run(self, request: "web.Request") -> "web.Response":
        ...
        if agent is not None:
            try:
                agent.interrupt("Stop requested via API", suppress_persist=True)
            except Exception:
                pass
    ...
    # ── MCP OAuth/CLI relay (aiohttp adapter over hermes_cli.mcp_oauth_relay) ──
    # These thin handlers reuse the relay's FastAPI handler functions directly
    # (they only raise fastapi.HTTPException, which we translate below) so the
    # OAuth/CLI logic stays in one place. The platform gateway proxies
    # /api/comworker/mcp/* here because the per-user container's api_server is
    # what is reachable on port 18080 (the web_server FastAPI app is not run in
    # `gateway run` mode).
    async def _handle_mcp_oauth_start(self, request: "web.Request") -> "web.Response":
        if not _RELAY_AVAILABLE:
            return web.json_response({"detail": "relay unavailable"}, status=501)
        auth_err = self._check_auth(request)
        ...
        try:
            ...
            body = None
        try:
            result = await _relay_oauth_start(name, body)
        except _FastAPIHTTPException as e:  # type: ignore[name-defined]
            return web.json_response({"detail": e.detail}, status=e.status_code)
    ...
    async def _handle_mcp_oauth_callback(self, request: "web.Request") -> "web.Response":
        ...
        try:
            result = await _relay_oauth_callback(name, code, state, error)
        except _FastAPIHTTPException as e:  # type: ignore[name-defined]
            return web.json_response({"detail": e.detail}, status=e.status_code)
        if isinstance(result, _FastAPIHTMLResponse):  # type: ignore[name-defined]
            return web.Response(
                body=result.body,
    ...
    async def _handle_mcp_oauth_poll(self, request: "web.Request") -> "web.Response":
        if not _RELAY_AVAILABLE:
            return web.json_response({"detail": "relay unavailable"}, status=501)
        auth_err = self._check_auth(request)
        if auth_err is not None:
        ...
        state = request.query.get("state", "")
        try:
            result = await _relay_oauth_poll(name, state)
        except _FastAPIHTTPException as e:  # type: ignore[name-defined]
            return web.json_response({"detail": e.detail}, status=e.status_code)
        return web.json_response(result)

    async def _handle_mcp_cli_install(self, request: "web.Request") -> "web.Response":
        if not _RELAY_AVAILABLE:
            return web.json_response({"detail": "relay unavailable"}, status=501)
        auth_err = self._check_auth(request)
        if auth_err is not None:
        ...
        name = request.match_info.get("name", "")
        try:
            result = await _relay_cli_install(name, request)
        except _FastAPIHTTPException as e:  # type: ignore[name-defined]
            return web.json_response({"detail": e.detail}, status=e.status_code)
        return web.json_response(result)

    async def _handle_mcp_cli_auth(self, request: "web.Request") -> "web.Response":
        if not _RELAY_AVAILABLE:
            return web.json_response({"detail": "relay unavailable"}, status=501)
        auth_err = self._check_auth(request)
        if auth_err is not None:
        ...
        name = request.match_info.get("name", "")
        try:
            result = await _relay_cli_auth(name, request)
        except _FastAPIHTTPException as e:  # type: ignore[name-defined]
            return web.json_response({"detail": e.detail}, status=e.status_code)
        return web.json_response(result)

    async def _handle_mcp_cli_status(self, request: "web.Request") -> "web.Response":
        if not _RELAY_AVAILABLE:
            return web.json_response({"detail": "relay unavailable"}, status=501)
        auth_err = self._check_auth(request)
        if auth_err is not None:
        ...
        name = request.match_info.get("name", "")
        try:
            result = await _relay_cli_status(name, request)
        except _FastAPIHTTPException as e:  # type: ignore[name-defined]
            return web.json_response({"detail": e.detail}, status=e.status_code)
        return web.json_response(result)
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # MCP OAuth/CLI relay endpoints (adapted from hermes_cli.mcp_oauth_relay).
            # The platform gateway forwards /api/comworker/mcp/* to this api_server,
            # where the relay performs PKCE/OAuth and CLI install/auth against the
            # connector's provider on behalf of the per-user container.
            if _RELAY_AVAILABLE:
                self._app.router.add_post("/api/mcp/{name}/oauth/start", self._handle_mcp_oauth_start)
                self._app.router.add_get("/api/mcp/{name}/oauth/callback", self._handle_mcp_oauth_callback)
                self._app.router.add_get("/api/mcp/{name}/oauth/poll", self._handle_mcp_oauth_poll)
                self._app.router.add_post("/api/mcp/{name}/cli/install", self._handle_mcp_cli_install)
                self._app.router.add_post("/api/mcp/{name}/cli/auth", self._handle_mcp_cli_auth)
                self._app.router.add_post("/api/mcp/{name}/cli/status", self._handle_mcp_cli_status)
