...
def build_claw_parser(subparsers, *, cmd_claw: Callable) -> None:
    ...
    claw_parser = subparsers.add_parser(
        "claw",
        help="ComWorker migration tools",
        description="Migrate settings, memories, skills, and API keys from ComWorker to Hermes",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from ComWorker to Hermes",
        description="Import settings, memories, skills, and API keys from an ComWorker installation. "
        "Always shows a preview before making changes.",
    )
    claw_migrate.add_argument(
        "--source", help="Path to ComWorker directory (default: ~/.comworker)"
    )
    claw_migrate.add_argument(
        "--dry-run",
        action="store_true",
    ...
    claw_cleanup = claw_subparsers.add_parser(
        "cleanup",
        aliases=["clean"],
        help="Archive leftover ComWorker directories after migration",
        description="Scan for and archive leftover ComWorker directories to prevent state fragmentation",
    )
    claw_cleanup.add_argument(
        "--source", help="Path to a specific ComWorker directory to clean up"
    )
    claw_cleanup.add_argument(
        "--dry-run",
        action="store_true",
