"""hermes claw — ComWorker migration commands.
...
Usage:
    ...
    hermes claw cleanup              # Archive leftover ComWorker directories
...
# Known ComWorker directory names (current + legacy)
_COMWORKER_DIR_NAMES = (".comworker", ".clawdbot", ".moltbot")

def _detect_comworker_processes() -> list[str]:
    """Detect running ComWorker processes and services.
    ...
    if sys.platform != "win32":
        try:
            result = subprocess.run(
                ["systemctl", "--user", "is-active", "comworker-gateway.service"],
                capture_output=True, text=True, timeout=5,
            )
            if result.stdout.strip() == "active":
                found.append("systemd service: comworker-gateway.service")
    ...
    if sys.platform == "win32":
        try:
            for exe in ("comworker.exe", "clawd.exe"):
                result = subprocess.run(
            ...
            # Node.js-hosted ComWorker — tasklist doesn't show command lines,
            # so fall back to PowerShell.
            ps_cmd = (
                'Get-CimInstance Win32_Process -Filter "Name = \'node.exe\'" | '
                'Where-Object { $_.CommandLine -match "comworker|clawd" } | '
            ...
            if result.stdout.strip():
                found.append(f"node.exe process with comworker in command line (PID {result.stdout.strip()})")
        except Exception:
            pass
    else:
        try:
            ...
            if result.returncode == 0:
                ...
                found.append(f"comworker process(es) (PIDs: {', '.join(pids)})")
        except (FileNotFoundError, subprocess.TimeoutExpired):
            pass
...
def _warn_if_comworker_running(auto_yes: bool) -> None:
    """Warn if ComWorker is still running before migration.

    Telegram, Discord, and Slack only allow one active connection per bot
    token. Migrating while ComWorker is running causes both to fight for the
    same token.
    """
    running = _detect_comworker_processes()
    if not running:
        return

    print()
    print_error("ComWorker appears to be running:")
    for detail in running:
        print_info(f"  * {detail}")
    print_info(
        "Messaging platforms (Telegram, Discord, Slack) only allow one "
        "active session per bot token. If you continue, both ComWorker and "
        "Hermes may try to use the same token, causing disconnects."
    )
    print_info("Recommendation: stop ComWorker before migrating.")
    print()
    if auto_yes:
        return
    ...
    if not prompt_yes_no("Continue anyway?", default=False):
        print_info("Migration cancelled. Stop ComWorker and try again.")
        sys.exit(0)
...
# State files commonly found in ComWorker workspace directories — listed
...
def _find_migration_script() -> Path | None:
    """Find the comworker_to_hermes.py script in known locations."""
    for candidate in [_COMWORKER_SCRIPT, _COMWORKER_SCRIPT_INSTALLED]:
        if candidate.exists():
            return candidate
    return None
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("comworker_to_hermes", script_path)
...
def _find_comworker_dirs() -> list[Path]:
    """Find all ComWorker directories on disk."""
    found = []
    for name in _COMWORKER_DIR_NAMES:
...
def _scan_workspace_state(source_dir: Path) -> list[tuple[Path, str]]:
    """Scan an ComWorker directory for workspace state files.
...
def _archive_directory(source_dir: Path, dry_run: bool = False) -> Path:
    """Rename an ComWorker directory to .pre-migration.
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print()
        print("Commands:")
        print("  migrate          Migrate settings from ComWorker to Hermes")
        print("  cleanup          Archive leftover ComWorker directories after migration")
        print()
        print("Run 'hermes claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the ComWorker → Hermes migration."""
    # Check current and legacy ComWorker directories
    explicit_source = getattr(args, "source", None)
    if explicit_source:
        source_dir = Path(explicit_source)
    else:
        source_dir = Path.home() / ".comworker"
    ...
    # via --migrate-secrets, even under --preset full.  This mirrors ComWorker's
    ...
    if not source_dir.is_dir():
        print()
        print_error(f"ComWorker directory not found: {source_dir}")
        print_info("Make sure your ComWorker installation is at the expected path.")
        print_info("You can specify a custom path: hermes claw migrate --source /path/to/.comworker")
        return
    ...
    if not script_path:
        ...
        print_info(f"  {_COMWORKER_SCRIPT}")
        print_info(f"  {_COMWORKER_SCRIPT_INSTALLED}")
        print_info("Make sure the comworker-migration skill is installed.")
        return
    ...
    # Check if ComWorker is still running — migrating tokens while both are
    # active will cause conflicts (e.g. Telegram 409).
    _warn_if_comworker_running(auto_yes)
    ...
    try:
        ...
        logger.debug("ComWorker migration error", exc_info=True)
        return
    ...
    try:
        ...
        logger.debug("ComWorker migration preview error", exc_info=True)
        return
    ...
    if preview_count == 0 and preview_conflicts == 0:
        print()
        print_info("Nothing to migrate from ComWorker.")
    ...
    # Modelled on ComWorker's assertConflictFreePlan() — apply is a safe no-op
    ...
    # restorable with `hermes import`.  Mirrors ComWorker's
...
def _cmd_cleanup(args):
    """Archive leftover ComWorker directories after migration.

    Scans for ComWorker directories that still exist after migration and offers
    ...
    # Find ComWorker directories
    if explicit_source:
        dirs_to_check = [Path(explicit_source)]
    else:
        dirs_to_check = _find_comworker_dirs()

    if not dirs_to_check:
        print()
        print_success("No ComWorker directories found. Nothing to clean up.")
        return

    # Warn if ComWorker is still running — archiving while the service is
    # active causes it to recreate an empty skeleton directory (#8502).
    running = _detect_comworker_processes()
    if running:
        print()
        print_error("ComWorker appears to be still running:")
        for detail in running:
            print_info(f"  * {detail}")
        print_info(
            "Archiving .comworker/ while the service is active may cause it to "
            "immediately recreate an empty skeleton directory, destroying your config."
        )
        print_info("Stop ComWorker first: systemctl --user stop comworker-gateway.service")
        print()
        if not auto_yes:
            if not sys.stdin.isatty():
                print_info("Non-interactive session — aborting. Stop ComWorker and re-run.")
                return
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop ComWorker first, then re-run: hermes claw cleanup")
                return
    ...
    if dry_run:
        ...
        print_success(
            f"Cleaned up {total_archived} ComWorker "
