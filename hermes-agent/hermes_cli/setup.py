...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    Used after ComWorker migration to detect which sections can be skipped.
...
def _load_comworker_migration_module():
    """Load the comworker_to_hermes migration script as a module.

    Returns the loaded module, or None if the script can't be loaded.
    """
    if not _COMWORKER_SCRIPT.exists():
        return None

    spec = importlib.util.spec_from_file_location(
        "comworker_to_hermes", _COMWORKER_SCRIPT
    )
...
# Config values may have different semantics between ComWorker and Hermes.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Hermes to use your ComWorker messaging channels",
    "telegram": "⚠ Telegram — this will point Hermes at your ComWorker Telegram bot",
    "slack": "⚠ Slack — this will point Hermes at your ComWorker Slack workspace",
    "discord": "⚠ Discord — this will point Hermes at your ComWorker Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Hermes at your ComWorker WhatsApp connection",
    "config": "⚠ Config values — ComWorker settings may not map 1:1 to Hermes equivalents",
    "soul": "⚠ Instruction file — may contain ComWorker-specific setup/restart procedures",
    "memory": "⚠ Memory/context file — may reference ComWorker-specific infrastructure",
    "context": "⚠ Context file — may contain ComWorker-specific instructions",
}
...
def _print_migration_preview(report: dict):
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: ComWorker config values may have different semantics in Hermes.", Colors.YELLOW))
        print(color("  For example, ComWorker's tool_call_execution: \"auto\" ≠ Hermes's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from ComWorker may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_comworker_migration(hermes_home: Path) -> bool:
    """Detect ~/.comworker and offer to migrate during first-time setup.
    ...
    """
    comworker_dir = Path.home() / ".comworker"
    if not comworker_dir.is_dir():
        return False
    ...
    print()
    print_header("ComWorker Installation Detected")
    print_info(f"Found ComWorker data at {comworker_dir}")
    ...
    try:
        mod = _load_comworker_migration_module()
        if mod is None:
            print_warning("Could not load migration script.")
            return False
    except Exception as e:
        print_warning(f"Could not load migration script: {e}")
        logger.debug("ComWorker migration module load error", exc_info=True)
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            source_root=comworker_dir.resolve(),
    ...
    if preview_count == 0:
        print()
        print_info("Nothing to import from ComWorker.")
        return False
    ...
    print()
    if migrated:
        print_success(f"Imported {migrated} item(s) from ComWorker.")
    if conflicts:
...
def run_setup_wizard(args):
    ...
    if is_existing:
        ...
        # missing items" flow (useful after a partial ComWorker migration
        ...
        # Offer ComWorker migration before configuration begins
        migration_ran = _offer_comworker_migration(hermes_home)
        if migration_ran:
            config = load_config()
    ...
    if migration_ran:
        print()
        print_info("Settings were imported from ComWorker.")
