#!/usr/bin/env python3
"""ComWorker 本地开发启动脚本（跨平台：macOS / Linux / Windows）。

一键启动所有本地开发服务：
  1. PostgreSQL (Docker 容器, 端口 5432)
  2. comworker bridge 后端 (端口 18080)
  3. platform gateway (端口 8080)
  4. manage 管理端 dev server (端口 3081)
...
用法:
  ...
  # 仅启动部分服务
  python start_local.py --only db,gateway,manage
...
def start_postgres() -> bool:
    ...
    result = subprocess.run(
        ["docker", "ps", "-q", "--filter", "name=^comworker-local-postgres$"],
    ...
    # 检查是否有已停止的容器
    result = subprocess.run(
        ["docker", "ps", "-aq", "--filter", "name=^comworker-local-postgres$"],
        capture_output=True, text=True,
    )
    if result.stdout.strip():
        log("启动已有的 PostgreSQL 容器...")
        subprocess.run(["docker", "start", "comworker-local-postgres"], check=True)
    else:
        log("创建新的 PostgreSQL 容器...")
        subprocess.run([
            "docker", "run", "-d",
            "--name", "comworker-local-postgres",
            "-e", "POSTGRES_USER=comworker",
            "-e", "POSTGRES_PASSWORD=comworker",
            "-e", "POSTGRES_DB=comworker_platform",
            "-v", "comworker-local-pgdata:/var/lib/postgresql/data",
            "-p", "5432:5432",
            "postgres:16-alpine",
        ], check=True)
    ...
    if wait_for_port(5432, timeout=15, name="PostgreSQL"):
        ...
        for attempt in range(1, 11):
            try:
                result = subprocess.run(
                    ["docker", "exec", "comworker-local-postgres", "psql", "-U", "comworker", "-d", "postgres", "-c", "SELECT 1"],
...
def stop_postgres():
    """停止 PostgreSQL 容器。"""
    subprocess.run(["docker", "stop", "comworker-local-postgres"], capture_output=True)
...
def start_bridge(env: dict) -> "subprocess.Popen | None":
    log("启动 ComWorker Bridge 后端 (端口 18080)...")

    if is_port_in_use(18080):
        warn("端口 18080 已被占用，跳过 bridge")
        return None

    bridge_dir = os.path.join(PROJECT_DIR, "comworker")
    ...
    for elapsed in range(1, timeout + 1):
        ...
        if proc.poll() is not None:
            ...
            error(f"ComWorker Bridge 启动失败 (exit code: {exit_code})")
            if output_lines:
                # 显示最后几行错误信息
                print()
        ...
        if is_port_in_use(18080):
            success("ComWorker Bridge 就绪 (端口 18080)")
            return proc
        ...
        if elapsed % 10 == 0:
            if elapsed <= 30:
                ...
                hint = "首次启动需要编译 comworker..."
            else:
                hint = "编译时间较长，请耐心等待..."
            sys.stdout.write(f"\r  等待 ComWorker Bridge... ({elapsed}/{timeout}s) {DIM}{hint}{RESET}  ")
            sys.stdout.flush()

    print()
    # 超时
    _drain_output(proc, output_lines, bridge_color)
    warn("ComWorker Bridge 启动超时 (120s)，继续启动其他服务")
    if output_lines:
...
def _suggest_bridge_fix(output_lines: list):
    ...
    if "cannot find module" in combined or "module not found" in combined:
        suggestions.append("尝试运行: cd comworker && pnpm install")
    ...
    if "syntaxerror" in combined or "typeerror" in combined:
        suggestions.append("代码错误，检查 comworker/bridge/ 下的源码")
    ...
    if not suggestions:
        ...
        suggestions.append("尝试手动运行: cd comworker && npx tsx bridge/start.ts")
...
def start_gateway(env: dict) -> "subprocess.Popen | None":
    ...
    proc_env = _base_env(
        PLATFORM_DATABASE_URL="postgresql+asyncpg://comworker:comworker@localhost:5432/comworker_platform",
        # 本地开发模式：直接代理到本机 comworker web，跳过 Docker 容器管理
        PLATFORM_DEV_COMWORKER_URL="http://127.0.0.1:18080",
        # WebSocket 直连 ComWorker Gateway（跳过 Bridge 的聊天中转）
...
def _stop_all_unix():
    patterns = ["bridge/start", "comworker gateway", "uvicorn app.main:app", "next.*3081"]
...
def _stop_all_windows():
    # 进程名 → 用 tasklist 过滤
    image_names = ["comworker.exe", "python.exe", "node.exe"]
...
def sync_deploy_copy():
    """将 deploy_copy 目录中的模板文件同步到 ~/.comworker/。

    仅在目标文件不存在时复制（不覆盖用户已有配置）。
    comworker_defaults.json 中的配置项会合并到 comworker.json（不覆盖已有项）。
    ...
    comworker_home = os.path.join(os.path.expanduser("~"), ".comworker")
    ...
    # 5. 合并 comworker_defaults.json 到 comworker.json
    defaults_path = os.path.join(deploy_dir, "comworker_defaults.json")
    config_path = os.path.join(comworker_home, "comworker.json")
    if os.path.isfile(defaults_path):
        _merge_comworker_defaults(defaults_path, config_path)

    if copied > 0:
        success(f"同步了 {copied} 个模板文件到 ~/.comworker/")
    else:
        success("deploy_copy 模板已就绪（无新文件需同步）")


def _sync_agents(src_agents_dir: str, comworker_home: str) -> int:
    """将 deploy_copy/Agents/ 下的每个子目录注册为独立 Agent。

    对每个 agent（如 hr, researcher, programmer）：
    1. 创建 ~/.comworker/agents/<name>/ 目录（供 gateway 磁盘发现）
    2. 创建 ~/.comworker/workspace-<name>/ 并同步 SOUL.md 等工作区文件
    3. 在 comworker.json 的 agents.list 中注册（workspace 路径 + 名称）
    """
    copied = 0
    config_path = os.path.join(comworker_home, "comworker.json")
    agents_to_register = []
    ...
    for entry in sorted(os.listdir(src_agents_dir)):
        ...
        agent_dir = os.path.join(comworker_home, "agents", agent_id)
        os.makedirs(agent_dir, exist_ok=True)

        # 2. 同步工作区文件到 workspace-<id>/
        workspace_dir = os.path.join(comworker_home, f"workspace-{agent_id}")
    ...
    # main 使用默认 workspace（~/.comworker/workspace），不需要 workspace-main
    main_agent_dir = os.path.join(comworker_home, "agents", "main")
    main_workspace = os.path.join(comworker_home, "workspace")
...
def _register_agents_in_config(config_path: str, agents: list):
    """将 agent 列表注册到 comworker.json 的 agents.list 中（不覆盖已有条目）。"""
...
def _merge_comworker_defaults(defaults_path: str, config_path: str):
    """将 defaults 中的配置项深度合并到 comworker.json（不覆盖已有叶子值）。

    如果 comworker.json 不存在，直接深拷贝 defaults 作为初始配置。
    """
    try:
    ...
    if not os.path.isfile(config_path):
        ...
        log("  从 comworker_defaults.json 创建 comworker.json")
        return
    ...
    if _deep_merge(config, defaults):
        with open(config_path, "w") as f:
            json.dump(config, f, indent=2, ensure_ascii=False)
        log("  深度合并 comworker_defaults.json → comworker.json")


# ── 主入口 ────────────────────────────────────────────────────────────

def main():
    parser = argparse.ArgumentParser(description="ComWorker 本地开发启动脚本")
    parser.add_argument("--stop", action="store_true", help="停止所有本地服务")
    parser.add_argument("--only", type=str, help="仅启动指定服务，逗号分隔 (db,bridge,gateway,manage)")
    ...
    # 解析要启动的服务
    all_services = ["db", "bridge", "gateway", "manage"]
    ...
    print(f"\n{BOLD}🔧 ComWorker 本地开发环境 ({platform_label}){RESET}\n")

    # 同步 deploy_copy 模板文件到 ~/.comworker/, 用于部署时初始化，方便新用户不必每次都安装
    sync_deploy_copy()
    print()
            ...
            for line in f:
                ...
                if line and not line.startswith("#") and "=" in line:
                    ...
                    if key == "DEFAULT_MODEL" and val:
                        ...
                        extra_env["COMWORKER_AGENTS__DEFAULTS__MODEL"] = model
    ...
    try:
        ...
        # 2. ComWorker Bridge 后端（含就绪等待，gateway 代理依赖它）
