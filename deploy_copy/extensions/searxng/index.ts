/**
 * ComWorker SearXNG Plugin
 *
 * Provides a `searxng_search` tool that queries a self-hosted SearXNG instance.
 * Zero invasion of core ComWorker code — works as a standard plugin extension.
 *
 * Configuration:
...
import type { ComWorkerPluginApi } from "comworker/plugin-sdk/searxng";
...
async function searxngSearch(params: {
  ...
  try {
    const res = await fetch(url.toString(), {
      headers: {
        "User-Agent": "ComWorker-SearXNG-Plugin/1.0",
...
const searxngPlugin = {
  ...
  register(api: ComWorkerPluginApi) {
