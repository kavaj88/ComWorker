#!/usr/bin/env python3
"""ComWorker 本地开发环境准备脚本（跨平台：macOS / Linux / Windows）。
...
检查并自动安装本地开发所需的全部依赖：
  ...
  9. ComWorker Bridge 依赖 (npm install)
  10. ComWorker 主项目依赖 (pnpm install)
...
def check_comworker_deps(fix: bool) -> CheckResult:
    """通过 uv sync 安装/验证 comworker 主项目依赖。"""
    # 快速检查：comworker 包是否已安装到当前 venv
    ...
    info("正在执行 uv sync（comworker 主项目）...")
...
def check_platform_deps(fix: bool) -> CheckResult:
    """检查 platform gateway (comworker-platform) 是否已安装到当前 venv。"""
    if _is_pkg_installed("comworker-platform"):
        return CheckResult(True, "comworker-platform 已安装")

    if not fix:
        return CheckResult(False, "comworker-platform 未安装，运行 prepare.py（不带 --check）自动安装")

    info("正在安装 platform 依赖（uv pip install -e platform/）...")
    r = run(*_uv_cmd(), "pip", "install", "-e", str(PLATFORM_DIR), capture=False)
    if r.returncode == 0 and _is_pkg_installed("comworker-platform"):
        return CheckResult(True, "comworker-platform 已安装", fixed=True)
...
def main():
    parser = argparse.ArgumentParser(description="ComWorker 开发环境准备脚本")
    ...
    print(f"\n{BOLD(f'🔧 ComWorker 开发环境准备 ({platform_label})')}")
    if args.check:
